...
const EyeSpeech = ({ theme, toggleTheme }) => {
  ...
  useEffect(() => {
    ...
    script.async = true;
    script.onload = () => console.log("WebGazer script loaded.");
  ...
  const startEyeTracking = async () => {
    if (!window.webgazer) {
      alert("WebGazer is still loading. Please wait.");
      return;
    }
  ...
  const startRecording = async () => {
    ...
    try {
      ...
      if (!devices.some((d) => d.kind === "audioinput")) throw new Error("No microphone detected.");
          ...
          <div className="floating w-full position-relative mx-auto playbook-binding">
            <div className="space-y-6 bg-white p-6 sm:p-8 md:p-10 flex flex-col rounded-2xl shadow-lg shadow-gray-700 gap-4 flex-grow transition-all hover:shadow-2xl h-full">
              ...
              <div className="space-y-4 w-full bg-blue-50 dark:bg-blue-900/30 p-6 rounded-xl border border-blue-200 dark:border-blue-700 mb-6 floating">
                ...
                <video ref={videoRef} autoPlay playsInline muted className="w-full aspect-video bg-gray-200 rounded-lg flex items-center justify-center text-gray-300 dark:text-gray-400 object-cover">
                  <p>CAMERA FEED</p>
                </video>
                <div className="flex flex-wrap gap-3">
                  <button onClick={startEyeTracking} disabled={isEyeTrackerRunning} className={`transition-all hover:shadow-2xl ${isEyeTrackerRunning ? "bg-gray-400 cursor-not-allowed" : "bg-blue-700 hover:bg-blue-800"} text-white px-4 py-2 rounded-lg shadow flex items-center gap-2 justify-center`}>
                    <span className="material-symbols-outlined">play_circle</span>
                    {isEyeTrackerRunning ? "Running..." : "Start Game"}
                  </button>
                  <button onClick={stopEyeTracking} disabled={!isEyeTrackerRunning} className="transition-all hover:shadow-2xl bg-red-600 text-white px-4 py-2 rounded-lg shadow hover:bg-red-700 flex items-center gap-2 justify-center">
                    <span className="material-symbols-outlined">stop_circle</span>
                    Stop Game
                  </button>
                  <button onClick={sendEyeData} disabled={gazePoints.length === 0 || isEyeTrackerRunning} className={`transition-all hover:shadow-2xl ${gazePoints.length === 0 || isEyeTrackerRunning ? "bg-gray-500 cursor-not-allowed" : "bg-green-600 hover:bg-green-700"} text-white px-4 py-2 rounded-lg shadow flex items-center gap-2 justify-center`}>
              ...
              <div className="space-y-4 bg-yellow-50 p-6 rounded-xl border border-yellow-400">
                ...
                <p className="text-sm text-gray-500 mt-2">Recording automatically stops after 10 seconds.</p>
                {audioUrl && (
            ...
            <div className="space-y-6 bg-green-50 p-6 rounded-xl border border-green-200 text-gray-700 flex-grow">
              ...
              <div className="space-y-4 text-lg">
                <div className="p-4 bg-white rounded-lg shadow-md">
                  ...
                  <span className="font-semibold text-2xl text-gray-800">{eyeResult?.error ? <span className="text-red-500">{eyeResult.error}</span> : eyeResult?.label ? <span className="text-blue-500">{eyeResult.label}</span> : "Not analyzed"}</span>
                </div>
                <div className="p-4 bg-white rounded-lg shadow-md">
